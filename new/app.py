...
def _progress(doc: dict) -> dict:
    ...
    if track == "maritime":
        ...
        b = doc["bundle"]
        base["formDone"] = bool((b.get("story") or "").strip()) or bool(b.get("tools"))
...
def assessment_state(token):
    ...
    return jsonify({
        ...
        "savedForm": {"story": b.get("story", ""), "tools": b.get("tools", [])},
        "savedTests": saved_tests,
        "videoAnswers": [{"qIndex": i, "done": True} for i, _ in enumerate(doc["video"].get("answers", []))],
    })
...
def submit_form(token):
    ...
    doc["bundle"]["formScore"] = calc_form_score(doc["bundle"])
    recompute(doc)
    db.save_candidate(doc)
    return jsonify({"success": True, "progress": _progress(doc)})
...
def submit_video(token):
    ...
    if "video" not in request.files:
        abort(400, "File video tidak ditemukan (field 'video').")
    ...
    # Transkripsi + penilaian bisa lambat → jalankan, lalu kembalikan status ringkas.
    video_pipeline.process_video_answer(doc["id"], q_index, question, fname, duration, doc["track"])
    doc = db.get_candidate(doc["id"])
    # Skor/ringkasan AI sengaja TIDAK dikembalikan ke kandidat (hanya untuk HR).
    return jsonify({"success": True, "answer": {"qIndex": q_index, "recorded": True},
                    "progress": _progress(doc)})
